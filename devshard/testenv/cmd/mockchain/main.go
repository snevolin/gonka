...
func main() {
	...
	restAddr := envOr("MOCK_CHAIN_REST_ADDR", ":1317")
	...
	restSrv, err := restface.NewServer(st, rpcSvc)
	if err != nil {
		log.Fatalf("mock-chain rest: %v", err)
	}
	...
	log.Printf("mock-chain REST listening on %s", restAddr)
	log.Printf("mock-chain testenv admin on %s", adminAddr)

	errCh := make(chan error, 4)
	go func() {
		errCh <- grpcface.Serve(ctx, grpcAddr, st)
	}()
	go func() {
		errCh <- rpcSvc.Serve(ctx, rpcAddr)
	}()
	go func() {
		errCh <- restSrv.Serve(ctx, restAddr)
	}()
	go func() {
